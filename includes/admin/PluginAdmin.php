...
if (!class_exists(PluginAdmin::class)) {
    class PluginAdmin
    {
        ...
        public function register(): void
        {
            ...
            add_filter('manage_edit-shop_order_columns', [__CLASS__, 'extendOrderView'], 10);
            add_action('manage_shop_order_posts_custom_column', [__CLASS__, 'getExtendedOrderValue'], 2);
        ...
        public static function extendOrderView($columns): array
        {
            $newColumns = (is_array($columns)) ? $columns : [];
            unset($newColumns['wc_actions']);

            $newColumns['merchant_reference_id'] = __('SmobilPay Merchant Reference ID', Plugin::DOMAIN_TEXT);
            $newColumns['order_transaction_id'] = __('SmobilPay Transaction ID', Plugin::DOMAIN_TEXT);

            $newColumns['wc_actions'] = $columns['wc_actions'];

            return $newColumns;
        }

        public static function getExtendedOrderValue($column)
        {
            global $post;
            $orderId = $post->ID;
